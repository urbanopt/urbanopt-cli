...
module URBANopt
  module CLI
    ...
    if @opthash.command == 'usg_complete'
      ...
      usg_cli_addition += " -o #{output_file}+_converted.csv --no-reports"
      ...
      final_output_file = output_file.sub(/\.csv$/, '_converted.csv')
      FileUtils.mv(final_output_file, output_file)
      intermediate_file = output_file.sub(/\.csv$/, '+_converted.csv')
      FileUtils.rm_f(intermediate_file)

      # puts
      puts "\nUSG processing complete. Output file: #{output_file}\n"
