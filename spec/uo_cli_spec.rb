RSpec.describe URBANopt::CLI do
  ...
  context "Admin" do
    it 'has the correct version number' do
      ...
      expect(URBANopt::CLI::VERSION).to eq("0.2.2")
    end
    ...
    it 'deletes a scenario' do
      ...
      expect(File.exists?(File.join(test_directory, "run", "two_building_scenario", "1", "data_point_out.json"))).to be true
      system("#{call_cli} -d -s #{test_scenario}")
      expect(File.exists?(File.join(test_directory, "run", "two_building_scenario", "1", "data_point_out.json"))).to be false
    end
  end
  ...
  context "Create project" do
    ...
    it 'creates an example project directory' do
      ...
      expect(File.exists?(test_feature)).to be true
    end

    it 'creates an empty project directory' do
      system("#{call_cli} -e -p #{test_directory}")
      expect(File.exists?(test_feature)).to be false
  ...
  context "Post-process a scenario" do
    ...
    it "opendss post-processes a scenario" do
      ...
      expect(File.exists?(File.join(test_directory, "run", "two_building_scenario", "1", "feature_reports", "default_feature_report_opendss.csv"))).to be true
    end
  end
end
