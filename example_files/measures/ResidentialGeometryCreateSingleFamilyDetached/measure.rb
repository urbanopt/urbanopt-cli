...
class CreateResidentialSingleFamilyDetachedGeometry < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    if (foundation_type == 'pier and beam') && (foundation_height <= 0.0)
    ...
    # error checking
    if (garage_protrusion > 0) && (roof_type == WholeBuildingConstants.RoofTypeHip) && has_garage
      runner.registerError('Cannot handle protruding garage and hip roof.')
      return false
    end
    if (garage_protrusion > 0) && (aspect_ratio < 1) && has_garage && (roof_type == WholeBuildingConstants.RoofTypeGable)
      runner.registerError('Cannot handle protruding garage and attic ridge running from front to back.')
      return false
    end
    if (foundation_type == 'pier and beam') && has_garage
    ...
    if has_garage && (roof_type != WholeBuildingConstants.RoofTypeFlat)
      if num_floors > 1
      ...
      space_with_roof_over_garage.surfaces.each do |surface|
        if surface.surfaceType.casecmp('roofceiling').zero? && surface.outsideBoundaryCondition.casecmp('outdoors').zero?
          n_points = []
          s_points = []
          ...
          garage_attic_space.surfaces.each do |surface|
            if (num_floors > 1) || (attic_type == 'finished attic')
              m = WholeBuildingGeometry.initialize_transformation_matrix(OpenStudio::Matrix.new(4, 4, 0))
    ...
    model.getSpaces.each do |space|
      next if !WholeBuildingGeometry.space_is_finished(space)
