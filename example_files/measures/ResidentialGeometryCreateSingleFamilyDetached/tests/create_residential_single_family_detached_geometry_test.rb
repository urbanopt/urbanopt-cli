...
class CreateResidentialSingleFamilyDetachedGeometryTest < MiniTest::Test
  def test_error_existing_geometry
    ...
    assert_includes(result.errors.map(&:logMessage), 'Starting model is not empty.')
  end
  ...
  def test_argument_error_aspect_ratio_invalid
    ...
    assert_includes(result.errors.map(&:logMessage), 'Invalid aspect ratio entered.')
  end
  ...
  def test_argument_error_pierbeam_height_invalid
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'The pier & beam height must be greater than 0 ft.')
  end
  ...
  def test_argument_error_pierbeam_with_garage
    ...
    args_hash['foundation_type'] = 'pier and beam'
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Cannot handle garages with a pier & beam foundation type.')
  end
  ...
  def test_argument_error_num_floors_invalid
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Too many floors.')
  end
  ...
  def test_argument_error_garage_protrusion_invalid
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Invalid garage protrusion value entered.')
  end
  ...
  def test_argument_error_hip_roof_and_garage_protrudes
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Cannot handle protruding garage and hip roof.')
  end
  ...
  def test_argument_error_living_and_garage_ridges_are_parallel
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Cannot handle protruding garage and attic ridge running from front to back.')
  end
  ...
  def test_argument_error_garage_width_exceeds_living_width
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Invalid living space and garage dimensions.')
  end
  ...
  def test_argument_error_beds_not_equal_to_baths
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bedroom elements specified inconsistent with number of bathroom elements specified.')
  end
  ...
  def test_argument_error_beds_not_equal_to_units
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bedroom elements specified inconsistent with number of multifamily units defined in the model.')
  end
  ...
  def test_argument_error_baths_not_equal_to_units
    ...
    args_hash['num_bathrooms'] = '2.0, 2.0'
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bathroom elements specified inconsistent with number of multifamily units defined in the model.')
  end
  ...
  def test_argument_error_beds_not_numerical
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bedrooms must be a numerical value.')
  end
  ...
  def test_argument_error_baths_not_numerical
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bathrooms must be a numerical value.')
  end
  ...
  def test_argument_error_beds_not_integer
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bedrooms must be a non-negative integer.')
  end
  ...
  def test_argument_error_baths_not_positive_multiple_of_0pt25
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Number of bathrooms must be a positive multiple of 0.25.')
  end
  ...
  def test_error_invalid_eaves_depth
    ...
    assert_includes(result.errors.map(&:logMessage), 'Eaves depth must be greater than or equal to 0.')
  end
  ...
  def test_error_invalid_neighbor_offset
    ...
    assert(result.errors.size == 1)
    assert_equal('Fail', result.value.valueName)
    assert_includes(result.errors.map(&:logMessage), 'Neighbor offsets must be greater than or equal to 0.')
  end
  ...
  def test_error_invalid_orientation
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Invalid orientation entered.')
  end
  ...
  def _test_measure(osm_file_or_model, args_hash, expected_num_del_objects, expected_num_new_objects, expected_values, test_name)
    ...
    all_new_objects.each do |obj_type, new_objects|
      new_objects.each do |new_object|
        next if !new_object.respond_to?("to_#{obj_type}")
        ...
        if obj_type == 'Space'
          if new_object.name.to_s.start_with?('garage attic space')
            ...
            actual_values['UnfinishedAtticFloorArea'] += UnitConversions.convert(new_object.floorArea, 'm^2', 'ft^2')
          elsif new_object.name.to_s.start_with?('finished attic', 'garage finished attic')
