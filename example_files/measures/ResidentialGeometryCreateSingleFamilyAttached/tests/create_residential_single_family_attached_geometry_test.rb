...
class CreateResidentialSingleFamilyAttachedGeometryTest < MiniTest::Test
  ...
  def test_argument_error_crawl_height_invalid
    ...
    assert_includes(result.errors.map(&:logMessage), 'The crawlspace height can be set between 1.5 and 5 ft.')
  end
  ...
  def test_argument_error_odd_and_rear_units
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Specified a building with rear units and an odd number of units.')
  end
  ...
  def test_one_unit_per_floor_with_rear_units
    ...
    args_hash['has_rear_units'] = 'true'
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), "Specified building as having rear units, but didn't specify enough units.")
  end
    ...
    all_new_objects.each do |obj_type, new_objects|
      new_objects.each do |new_object|
        ...
        if obj_type == 'Space'
          ...
          elsif new_object.outsideBoundaryCondition.casecmp('adiabatic').zero?
            actual_values['NumAdiabaticSurfaces'] += 1
          end
        end
      end
