...
class CreateResidentialMultifamilyGeometryTest < MiniTest::Test
  ...
  def test_error_num_units
    ...
    assert_includes(result.errors.map(&:logMessage), 'The number of units must be divisible by the number of floors.')
  end
  ...
  def test_error_no_corr
    ...
    result = _test_error(nil, args_hash)
    assert_includes(result.errors.map(&:logMessage), 'Invalid corridor width entered.')
  end
