...
class MiscLoads
  ...
  def self.apply_pool_or_hot_tub_pump(model, pool_or_hot_tub, obj_name, living_space)
    ...
    unless pool_or_hot_tub.pump_kwh_per_year.nil?
  ...
  def self.get_pool_pump_default_values(cfa, nbeds)
    158.6 / 0.070 * (0.5 + 0.25 * nbeds / 3.0 + 0.25 * cfa / 1920.0) # kWh/yr
  end
  ...
  def self.get_pool_heater_default_values(cfa, nbeds, type)
    ...
    [load_units, load_value]
  end

  def self.get_hot_tub_pump_default_values(cfa, nbeds)
    59.5 / 0.059 * (0.5 + 0.25 * nbeds / 3.0 + 0.25 * cfa / 1920.0) # kWh/yr
  end
  ...
  def self.get_electric_vehicle_charging_default_values
    ...
    vehicle_annual_miles_driven * vehicle_kWh_per_mile / (ev_charger_efficiency * ev_battery_efficiency) # kWh/yr
  end

  def self.get_well_pump_default_values(cfa, nbeds)
    50.8 / 0.127 * (0.5 + 0.25 * nbeds / 3.0 + 0.25 * cfa / 1920.0) # kWh/yr
  end

  def self.get_gas_grill_default_values(cfa, nbeds)
    0.87 / 0.029 * (0.5 + 0.25 * nbeds / 3.0 + 0.25 * cfa / 1920.0) # therm/yr
  end

  def self.get_gas_lighting_default_values(cfa, nbeds)
    0.22 / 0.012 * (0.5 + 0.25 * nbeds / 3.0 + 0.25 * cfa / 1920.0) # therm/yr
  end

  def self.get_gas_fireplace_default_values(cfa, nbeds)
    1.95 / 0.032 * (0.5 + 0.25 * nbeds / 3.0 + 0.25 * cfa / 1920.0) # therm/yr
  end
end
