...
class Airflow
  ...
  def self.get_default_vented_attic_sla
    1.0 / 300.0 # Table 4.2.2(1) - Attics
  end

  def self.get_default_vented_crawl_sla
    1.0 / 150.0 # Table 4.2.2(1) - Crawlspaces
  end
  ...
  def self.get_default_mech_vent_fan_power(vent_fan)
    ...
    if vent_fan.is_shared_system
      ...
      raise "Unexpected fan_type: '#{fan_type}'."
    end
  end
  ...
  def self.set_wind_speed_correction(model, site_type, shelter_coef, min_neighbor_distance)
    ...
    # Local Shielding
    wind_speed.S_wo = if shelter_coef.nil?
  ...
  def self.apply_natural_ventilation_and_whole_house_fan(model, _weather, vent_fans_whf, open_window_area, nv_clg_ssn_sensor)
    ...
    whf_zone = nil
    if !@spaces[HPXML::LocationAtticVented].nil?
      whf_zone = @spaces[HPXML::LocationAtticVented].thermalZone.get
    elsif !@spaces[HPXML::LocationAtticUnvented].nil?
    ...
    if !htg_sp_sensor.nil? && !clg_sp_sensor.nil?
  ...
  def self.create_nv_and_whf_avail_sch(model, obj_name, num_days_per_week)
    ...
    method_array = %w[setApplyMonday setApplyWednesday setApplyFriday setApplySaturday setApplyTuesday setApplyThursday setApplySunday]
    for i in 1..7 do
      on_rule.public_send(method_array[i - 1], true) if num_days_per_week >= i
    end
  ...
  def self.create_other_equipment_object_and_actuator(model:, name:, space:, frac_lat:, frac_lost:, hpxml_fuel_type: nil, end_use: nil, is_duct_load_for_report: nil)
    ...
    other_equip.setEndUseSubcategory(end_use) unless end_use.nil?
    ...
    unless is_duct_load_for_report.nil?
  ...
  def self.initialize_cfis(model, vent_fans_mech, hvac_map)
    ...
    vent_fans_mech.each do |vent_mech|
      ...
      cfis_sys_ids = vent_mech.distribution_system.hvac_systems.map(&:id)
      ...
      hvac_map.each do |sys_id, hvacs|
        ...
        hvacs.each do |loop|
          ...
          next if !@cfis_airloop[vent_mech.id].nil? && (@cfis_airloop[vent_mech.id] == loop) # already assigned

          raise 'Two airloops found for CFIS.' unless @cfis_airloop[vent_mech.id].nil?

          @cfis_airloop[vent_mech.id] = loop
        end
      end

      @cfis_t_sum_open_var[vent_mech.id] = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{Constants.ObjectNameMechanicalVentilation.tr(' ', '_')}_cfis_t_sum_open_#{index}") # Sums the time during an hour the CFIS damper has been open
      @cfis_f_damper_extra_open_var[vent_mech.id] = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{Constants.ObjectNameMechanicalVentilation.tr(' ', '_')}_cfis_f_extra_damper_open_#{index}") # Fraction of timestep the CFIS blower is running while hvac is not operating. Used by infiltration and duct leakage programs
  ...
  def self.initialize_air_loop_objects(model, air_loop)
    ...
    supply_fan = if system.nil? # Evaporative cooler supply fan directly on air loop
                   air_loop.supplyFan.get
                 else
                   system.supplyFan.get
                 end

    @fan_rtf_var[air_loop] = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{air_loop.name} Fan RTF".tr(' ', '_'))
  ...
  def self.initialize_fan_coil_objects(model, fan_coil)
    ...
    @fan_rtf_var[fan_coil] = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{fan_coil.name} Fan RTF".tr(' ', '_'))
  ...
  def self.apply_ducts(model, ducts, object)
    ducts.each do |duct|
      if duct.leakage_frac.nil? == duct.leakage_cfm25.nil?
        raise 'Ducts: Must provide either leakage fraction or cfm25, but not both.'
      end
      if !duct.leakage_frac.nil? && ((duct.leakage_frac < 0) || (duct.leakage_frac > 1))
        raise 'Ducts: Leakage Fraction must be greater than or equal to 0 and less than or equal to 1.'
      end
      if !duct.leakage_cfm25.nil? && (duct.leakage_cfm25 < 0)
        raise 'Ducts: Leakage CFM25 must be greater than or equal to 0.'
      end
      if duct.rvalue < 0
        raise 'Ducts: Insulation Nominal R-Value must be greater than or equal to 0.'
      end
      if duct.area < 0
        raise 'Ducts: Surface Area must be greater than or equal to 0.'
      end
    end

    ducts.each do |duct|
      duct.rvalue = get_duct_insulation_rvalue(duct.rvalue, duct.side) # Convert from nominal to actual R-value
      if !duct.loc_schedule.nil?
        # Pass MF space temperature schedule name
        duct.location = duct.loc_schedule.name.to_s
      elsif !duct.loc_space.nil?
    ...
    unless ducts.empty?
      ...
      object.additionalProperties.setFeature(Constants.SizingInfoDuctSides, ducts.map(&:side).join(','))
    ...
    return if ducts.empty? # No ducts

    # get duct located zone or ambient temperature schedule objects
    duct_locations = ducts.map { |duct| duct.zone.nil? ? duct.loc_schedule : duct.zone }.uniq
    ...
    ah_mfr_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object.name} AH MFR".tr(' ', '_'))
    ...
    ah_vfr_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object.name} AH VFR".tr(' ', '_'))
    ...
    ah_tout_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object.name} AH Tout".tr(' ', '_'))
    ...
    ah_wout_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object.name} AH Wout".tr(' ', '_'))
    ...
    ra_t_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object.name} RA T".tr(' ', '_'))
    if !living_zone_return_air_node.nil?
    ...
    ra_w_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object.name} Ra W".tr(' ', '_'))
    if !living_zone_return_air_node.nil?
    ...
    duct_locations.each_with_index do |duct_location, i|
      next if !duct_location.nil? && (duct_location.name.to_s == @living_zone.name.to_s)
      ...
      dz_t_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object_name_idx} DZ T".tr(' ', '_'))
      if duct_location.is_a? OpenStudio::Model::ThermalZone
        ...
        raise 'Unexpected duct zone type passed'
      end
      dz_t_sensor.setName("#{dz_t_var.name} s")

      # Duct zone humidity ratio
      dz_w_var = OpenStudio::Model::EnergyManagementSystemGlobalVariable.new(model, "#{object_name_idx} DZ W".tr(' ', '_'))
      if duct_location.is_a? OpenStudio::Model::ThermalZone
      ...
      equip_act_infos << if !duct_location.is_a? OpenStudio::Model::ThermalZone # Outside or scheduled temperature
                           ['supply_cond_to_dz', 'SupCondToDZ', false, @living_space, 0.0, 1.0] # Arbitrary space, all heat lost
                         else
                           ['supply_cond_to_dz', 'SupCondToDZ', false, duct_location.spaces[0], 0.0, 0.0]
                         end

      # Return duct conduction impact on the duct zone
      equip_act_infos << if !duct_location.is_a? OpenStudio::Model::ThermalZone # Outside or scheduled temperature
                           ['return_cond_to_dz', 'RetCondToDZ', false, @living_space, 0.0, 1.0] # Arbitrary space, all heat lost
                         else
                           ['return_cond_to_dz', 'RetCondToDZ', false, duct_location.spaces[0], 0.0, 0.0]
                         end

      # Supply duct sensible leakage impact on the duct zone
      equip_act_infos << if !duct_location.is_a? OpenStudio::Model::ThermalZone # Outside or scheduled temperature
                           ['supply_sens_lk_to_dz', 'SupSensLkToDZ', false, @living_space, 0.0, 1.0] # Arbitrary space, all heat lost
                         else
                           ['supply_sens_lk_to_dz', 'SupSensLkToDZ', false, duct_location.spaces[0], 0.0, 0.0]
                         end

      # Supply duct latent leakage impact on the duct zone
      equip_act_infos << if !duct_location.is_a? OpenStudio::Model::ThermalZone # Outside or scheduled temperature
                           ['supply_lat_lk_to_dz', 'SupLatLkToDZ', false, @living_space, 0.0, 1.0] # Arbitrary space, all heat lost
                         else
                           ['supply_lat_lk_to_dz', 'SupLatLkToDZ', false, duct_location.spaces[0], 1.0, 0.0]
                         end
      ...
      [false, true].each do |is_cfis|
        ...
        equip_act_infos.each do |act_info|
          ...
          object_name = "#{object_name_idx} #{prefix}#{act_info[1]}".tr(' ', '_')
      ...
      if duct_location.is_a? OpenStudio::Model::ThermalZone # in a space
        if !@spaces[HPXML::LocationBasementUnconditioned].nil? && (@spaces[HPXML::LocationBasementUnconditioned].thermalZone.get.name.to_s == duct_location.name.to_s)
          f_oa = 0.0
        elsif !@spaces[HPXML::LocationCrawlspaceUnvented].nil? && (@spaces[HPXML::LocationCrawlspaceUnvented].thermalZone.get.name.to_s == duct_location.name.to_s)
          f_oa = 0.0
        elsif !@spaces[HPXML::LocationAtticUnvented].nil? && (@spaces[HPXML::LocationAtticUnvented].thermalZone.get.name.to_s == duct_location.name.to_s)
          f_oa = 0.0
        end
      end
      ...
      if !leakage_fracs[HPXML::DuctTypeSupply].nil?
        duct_subroutine.addLine("  Set f_sup = #{leakage_fracs[HPXML::DuctTypeSupply]}") # frac
      elsif !leakage_cfm25s[HPXML::DuctTypeSupply].nil?
      ...
      if !leakage_fracs[HPXML::DuctTypeReturn].nil?
        duct_subroutine.addLine("  Set f_ret = #{leakage_fracs[HPXML::DuctTypeReturn]}") # frac
      elsif !leakage_cfm25s[HPXML::DuctTypeReturn].nil?
      ...
      unless duct_actuators['liv_to_dz_flow_rate'].nil?
        duct_subroutine.addLine("Set #{duct_vars['liv_to_dz_flow_rate'].name} = ZoneMixLvToDZ")
      end
      unless duct_actuators['dz_to_liv_flow_rate'].nil?
  ...
  def self.apply_infiltration_to_unconditioned_basement(model, _weather)
  ...
  def self.apply_infiltration_to_vented_attic(model, weather, vented_attic)
    ...
    if !vented_attic.vented_attic_sla.nil?
      vented_attic_sla = vented_attic.vented_attic_sla
    elsif !vented_attic.vented_attic_ach.nil?
    ...
    if !vented_attic_sla.nil?
      ...
      c_w_SG, c_s_SG = calc_wind_stack_coeffs(hor_lk_frac, neutral_level, space)
      apply_infiltration_to_unconditioned_space(model, space, nil, ela, c_w_SG, c_s_SG)
    elsif !vented_attic_const_ach.nil?
  ...
  def self.apply_local_ventilation(model, vent_object_array, obj_type_name)
    ...
    vent_object_array.each_with_index do |vent_object, index|
      ...
      for hr in 1..(vent_object.hours_in_operation.ceil)
        daily_sch[(vent_object.start_hour + hr - 1) % 24] = if remaining_hrs >= 1
                                                              1.0
                                                            else
                                                              remaining_hrs
                                                            end
  ...
  def self.calc_hrv_erv_effectiveness(vent_mech_fans)
    ...
    vent_mech_fans.each do |vent_mech|
      ...
      if vent_mech_cfm > 0
        ...
        if !vent_mech.sensible_recovery_efficiency.nil?
        ...
        if (vent_mech_sens_eff < 0.0) || (vent_mech_sens_eff > 1.0)
          raise "The calculated ERV/HRV sensible effectiveness is #{vent_mech_sens_eff} but should be between 0 and 1. Please revise ERV/HRV efficiency values."
        end
        ...
        if !vent_mech.total_recovery_efficiency.nil? || !vent_mech.total_recovery_efficiency_adjusted.nil?
          ...
          h_sup_out = if !vent_mech.total_recovery_efficiency.nil?
                        ...
                        h_sup_in - (vent_mech.total_recovery_efficiency * (m_fan * (h_sup_in - h_exh_in) + p_fan) + p_fan) / m_fan
                      else
                        # The following is derived from (taken from CSA 439, Clause 9.2.1, Eq. 7):
                        h_sup_in - (vent_mech.total_recovery_efficiency_adjusted * (h_sup_in - h_exh_in))
                      end
          ...
          if (vent_mech_lat_eff < 0.0) || (vent_mech_lat_eff > 1.0)
            raise "The calculated ERV/HRV latent effectiveness is #{vent_mech_lat_eff} but should be between 0 and 1. Please revise ERV/HRV efficiency values."
          end
  ...
  def self.setup_mech_vent_vars_actuators(model:, program:)
    ...
    [fan_sens_load_actuator, fan_lat_load_actuator]
  end
  ...
  def self.calculate_fan_loads(_model, infil_program, vent_mech_erv_hrv_tot, hrv_erv_effectiveness_map, fan_sens_load_actuator, fan_lat_load_actuator, q_var, preconditioned = false)
    ...
    if !vent_mech_erv_hrv_tot.empty?
  ...
  def self.calculate_precond_loads(model, infil_program, vent_mech_preheat, vent_mech_precool, hrv_erv_effectiveness_map, fan_sens_load_actuator, fan_lat_load_actuator, hvac_map)
    ...
    vent_mech_preheat.each_with_index do |f_preheat, i|
      ...
      vent_mech_erv_hrv_tot = if [HPXML::MechVentTypeERV, HPXML::MechVentTypeHRV].include? f_preheat.fan_type
                                [f_preheat]
                              else
                                []
                              end
    ...
    vent_mech_precool.each_with_index do |f_precool, i|
      ...
      vent_mech_erv_hrv_tot = if [HPXML::MechVentTypeERV, HPXML::MechVentTypeHRV].include? f_precool.fan_type
                                [f_precool]
                              else
                                []
                              end
  ...
  def self.apply_mechanical_ventilation(model, vent_fans_mech, living_ach50, living_const_ach, weather, vent_fans_kitchen, vent_fans_bath, range_sch_sensors_map, bath_sch_sensors_map, has_flue_chimney, hvac_map)
    ...
    vent_mech_preheat = vent_fans_mech.select { |vent_mech| vent_mech.preheating_efficiency_cop.nil? }
    vent_mech_precool = vent_fans_mech.select { |vent_mech| vent_mech.precooling_efficiency_cop.nil? }
    vent_mech_shared = vent_fans_mech.select(&:is_shared_system)
    ...
    # Non-CFIS fan power
    sup_vent_mech_fan_w = vent_mech_sup_tot.map(&:average_unit_fan_power).sum(0.0)
    exh_vent_mech_fan_w = vent_mech_exh_tot.map(&:average_unit_fan_power).sum(0.0)
    # ERV/HRV and balanced system fan power combined altogether
    bal_vent_mech_fan_w = (vent_mech_bal_tot + vent_mech_erv_hrv_tot).map(&:average_unit_fan_power).sum(0.0)
    ...
    fan_heat_lost_fraction = if total_sup_exh_bal_w > 0.0
                               (1.0 * exh_vent_mech_fan_w + 0.0 * sup_vent_mech_fan_w + 0.5 * bal_vent_mech_fan_w) / total_sup_exh_bal_w
                             else
                               1.0
                             end
    ...
    sup_cfm_tot = vent_mech_sup_tot.map(&:average_total_unit_flow_rate).sum(0.0)
    exh_cfm_tot = vent_mech_exh_tot.map(&:average_total_unit_flow_rate).sum(0.0)
    bal_cfm_tot = vent_mech_bal_tot.map(&:average_total_unit_flow_rate).sum(0.0)
    erv_hrv_cfm_tot = vent_mech_erv_hrv_tot.map(&:average_total_unit_flow_rate).sum(0.0)
    cfis_cfm_tot = vent_mech_cfis_tot.map(&:average_total_unit_flow_rate).sum(0.0)
    ...
    model.getBuilding.additionalProperties.setFeature(Constants.SizingInfoMechVentExist, !vent_fans_mech.empty?)
  ...
  def self.apply_infiltration_to_living(living_ach50, living_const_ach, infil_program, weather, has_flue_chimney)
    if living_ach50.to_f > 0
      ...
      if !@spaces[HPXML::LocationCrawlspaceVented].nil?
  ...
  def self.calc_wind_stack_coeffs(hor_lk_frac, neutral_level, space, space_height = nil)
    space_height = Geometry.get_height_of_spaces([space]) if space_height.nil?
  ...
  def self.get_infiltration_NL_from_SLA(sla, infil_height)
    ...
    1000.0 * sla * (infil_height / 8.202)**0.4
  end
  ...
  def self.get_infiltration_SLA_from_ACH50(ach50, n_i, conditionedFloorArea, conditionedVolume, pressure_difference_Pa = 50)
    ...
    ((ach50 * 0.283316478 * 4.0**n_i * conditionedVolume) / (conditionedFloorArea * UnitConversions.convert(1.0, 'ft^2', 'in^2') * pressure_difference_Pa**n_i * 60.0))
  end
  ...
  def self.get_infiltration_ACH50_from_SLA(sla, n_i, conditionedFloorArea, conditionedVolume, pressure_difference_Pa = 50)
    ...
    ((sla * conditionedFloorArea * UnitConversions.convert(1.0, 'ft^2', 'in^2') * pressure_difference_Pa**n_i * 60.0) / (0.283316478 * 4.0**n_i * conditionedVolume))
  end

  def self.calc_duct_leakage_at_diff_pressure(q_old, p_old, p_new)
    q_old * (p_new / p_old)**0.6 # Derived from Equation C-1 (Annex C), p34, ASHRAE Standard 152-2004.
  end

  def self.calc_air_leakage_at_diff_pressure(n_i, q_old, p_old, p_new)
    q_old * (p_new / p_old)**n_i
  end
  ...
  def self.get_duct_insulation_rvalue(nominal_rvalue, side)
    ...
    return 1.7 if nominal_rvalue <= 0
  ...
  def self.get_mech_vent_whole_house_cfm(frac622, num_beds, cfa, std)
    ...
    return frac622 * ((num_beds + 1.0) * 7.5 + 0.03 * cfa) if std == '2013'

    frac622 * ((num_beds + 1.0) * 7.5 + 0.01 * cfa)
  end
end
