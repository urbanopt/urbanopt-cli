...
class Material
  ...
  def initialize(name = nil, thick_in = nil, mat_base = nil, k_in = nil, rho = nil, cp = nil, tAbs = nil, sAbs = nil, vAbs = nil, rvalue = nil)
    ...
    if !mat_base.nil?
      ...
      @k = unless mat_base.k_in.nil?
             UnitConversions.convert(mat_base.k_in, 'in', 'ft') # Btu/h-ft-F
           end
      @rho = mat_base.rho
      @cp = mat_base.cp
    else
    ...
    if !rvalue.nil?
      ...
      @rvalue = if @k_in > 0
                  @thick_in / @k_in # h-ft^2-F/Btu
                else
                  @thick_in / 10_000_000.0 # h-ft^2-F/Btu
                end
    end
  end
  ...
  def self.AirCavityClosed(thick_in)
    ...
    new(name = nil, thick_in = thick_in, mat_base = nil, k_in = thick_in / rvalue, rho = Gas.Air.rho, cp = Gas.Air.cp)
  end

  def self.AirCavityOpen(thick_in)
    new(name = nil, thick_in = thick_in, mat_base = nil, k_in = 10_000_000.0, rho = Gas.Air.rho, cp = Gas.Air.cp)
  end

  def self.AirFilm(rvalue)
    new(name = Constants.AirFilm, thick_in = 1.0, mat_base = nil, k_in = 1.0 / rvalue)
  end
  ...
  def self.CoveringBare(floorFraction = 0.8, rvalue = 2.08)
    ...
    thickness = 0.5 # in
    new(name = 'Floor Covering', thick_in = thickness, mat_base = nil, k_in = thickness / (rvalue * floorFraction), rho = 3.4, cp = 0.32, tAbs = 0.9, sAbs = 0.9, vAbs = 0.9)
  end

  def self.Concrete(thick_in)
    new(name = "Concrete #{thick_in} in.", thick_in = thick_in, mat_base = BaseMaterial.Concrete, k_in = nil, rho = nil, cp = nil, tAbs = 0.9)
  end
  ...
  def self.ExteriorFinishMaterial(siding, emittance, solar_absorptance, thick_in = nil)
    if siding == HPXML::SidingTypeWood
      ...
      new(siding, thick_in, nil, 0.71, 34.0, 0.28, emittance, solar_absorptance, solar_absorptance)
    elsif siding == HPXML::SidingTypeVinyl
      thick_in = 0.375 if thick_in.nil?
      new(siding, thick_in, BaseMaterial.Vinyl, nil, nil, nil, emittance, solar_absorptance, solar_absorptance)
    elsif siding == HPXML::SidingTypeStucco
      thick_in = 1.0 if thick_in.nil?
      new(siding, thick_in, BaseMaterial.Stucco, nil, nil, nil, emittance, solar_absorptance, solar_absorptance)
    elsif siding == HPXML::SidingTypeFiberCement
      thick_in = 0.375 if thick_in.nil?
      new(siding, thick_in, nil, 1.79, 21.7, 0.24, emittance, solar_absorptance, solar_absorptance)
    elsif siding == HPXML::SidingTypeBrick
      thick_in = 4.0 if thick_in.nil?
      new(siding, thick_in, BaseMaterial.Brick, nil, nil, nil, emittance, solar_absorptance, solar_absorptance)
    elsif siding == HPXML::SidingTypeAluminum
      thick_in = 0.375 if thick_in.nil?
      new(siding, thick_in, BaseMaterial.Aluminum, nil, nil, nil, emittance, solar_absorptance, solar_absorptance)
    end
  end

  def self.FloorWood
    Material.new(name = 'Wood Floor', thick_in = 0.625, mat_base = nil, k_in = 0.8004, rho = 34.0, cp = 0.29)
  end

  def self.GypsumWall(thick_in)
    new(name = "Drywall #{thick_in} in.", thick_in = thick_in, mat_base = BaseMaterial.Gypsum, k_in = nil, rho = nil, cp = nil, tAbs = 0.9, sAbs = 0.5, vAbs = 0.1)
  end

  def self.GypsumCeiling(thick_in)
    new(name = "Drywall #{thick_in} in.", thick_in = thick_in, mat_base = BaseMaterial.Gypsum, k_in = nil, rho = nil, cp = nil, tAbs = 0.9, sAbs = 0.3, vAbs = 0.1)
  end

  def self.Soil(thick_in)
    new(name = "Soil #{thick_in} in.", thick_in = thick_in, mat_base = BaseMaterial.Soil)
  end

  def self.Stud2x(thick_in)
    new(name = "Stud 2x #{thick_in} in.", thick_in = thick_in, mat_base = BaseMaterial.Wood)
  end

  def self.Stud2x4
    new(name = 'Stud 2x4', thick_in = 3.5, mat_base = BaseMaterial.Wood)
  end

  def self.Stud2x6
    new(name = 'Stud 2x6', thick_in = 5.5, mat_base = BaseMaterial.Wood)
  end

  def self.Stud2x8
    new(name = 'Stud 2x8', thick_in = 7.25, mat_base = BaseMaterial.Wood)
  end

  def self.Plywood(thick_in)
    new(name = "Plywood #{thick_in} in.", thick_in = thick_in, mat_base = BaseMaterial.Wood)
  end
  ...
  def self.RadiantBarrier(grade)
    ...
    new(name = 'Radiant Barrier', thick_in = 0.0084, mat_base = nil, k_in = 1629.6, rho = 168.6, cp = 0.22, tAbs = emittance, sAbs = 0.05, vAbs = 0.05)
  end
...
class BaseMaterial
  ...
  def self.Gypsum
    new(rho = 50.0, cp = 0.2, k_in = 1.1112)
  end

  def self.Wood
    new(rho = 32.0, cp = 0.29, k_in = 0.8004)
  end

  def self.Concrete
    new(rho = 140.0, cp = 0.2, k_in = 12.5)
  end

  def self.Gypcrete
    # http://www.maxxon.com/gyp-crete/data
    new(rho = 100.0, cp = 0.223, k_in = 4.7424)
  end

  def self.InsulationRigid
    new(rho = 2.0, cp = 0.29, k_in = 0.204)
  end

  def self.InsulationCelluloseDensepack
    new(rho = 3.5, cp = 0.25, k_in = nil)
  end

  def self.InsulationCelluloseLoosefill
    new(rho = 1.5, cp = 0.25, k_in = nil)
  end

  def self.InsulationFiberglassDensepack
    new(rho = 2.2, cp = 0.25, k_in = nil)
  end

  def self.InsulationFiberglassLoosefill
    new(rho = 0.5, cp = 0.25, k_in = nil)
  end

  def self.InsulationGenericDensepack
    new(rho = (self.InsulationFiberglassDensepack.rho + self.InsulationCelluloseDensepack.rho) / 2.0, cp = 0.25, k_in = nil)
  end

  def self.InsulationGenericLoosefill
    new(rho = (self.InsulationFiberglassLoosefill.rho + self.InsulationCelluloseLoosefill.rho) / 2.0, cp = 0.25, k_in = nil)
  end

  def self.Soil
    new(rho = 115.0, cp = 0.1, k_in = 12.0)
  end

  def self.Brick
    new(rho = 110.0, cp = 0.19, k_in = 5.5)
  end

  def self.Vinyl
    new(rho = 11.1, cp = 0.25, k_in = 0.62)
  end

  def self.Aluminum
    new(rho = 10.9, cp = 0.29, k_in = 0.61)
  end

  def self.Stucco
    new(rho = 80.0, cp = 0.21, k_in = 4.5)
  end
  ...
  def self.StrawBale
    new(rho = 11.1652, cp = 0.2991, k_in = 0.4164)
  end
end
...
class SimpleMaterial
  ...
  def self.Adiabatic
    new(name = 'Adiabatic', rvalue = 1000)
  end
end
...
class Liquid
  ...
  def self.H2O_l
    # From EES at STP
    new(62.32, 0.9991, 0.3386, 2.424, 1055, 32.0, 212.0, nil)
  end

  def self.R22_l
    # Converted from EnthDR22 f77 in ResAC (Brandemuehl)
    new(nil, 0.2732, nil, nil, 100.5, nil, -41.35, 204.9)
  end
end
...
class Gas
  ...
  def self.Air
    # From EES at STP
    new(0.07518, 0.2399, 0.01452, 0.04415, 28.97)
  end

  def self.AirGapRvalue
    1.0 # hr*ft*F/Btu (Assume for all air gap configurations since there is no correction for direction of heat flow in the simulation tools)
  end

  def self.H2O_v
    # From EES at STP
    new(nil, 0.4495, nil, nil, 18.02)
  end

  def self.R22_v
    # Converted from EnthDR22 f77 in ResAC (Brandemuehl)
    new(nil, 0.1697, nil, nil, nil)
  end

  def self.PsychMassRat
    self.H2O_v.m / self.Air.m
  end
end
