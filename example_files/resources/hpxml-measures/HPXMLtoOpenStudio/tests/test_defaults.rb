...
class HPXMLtoOpenStudioDefaultsTest < MiniTest::Test
  ...
  def test_building_construction
    ...
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_building_construction_values(hpxml_default, 21_600, true, 4)
    ...
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_building_construction_values(hpxml_default, 27_000, false, 2)
  end
  ...
  def test_infiltration
    # Test inputs not overridden by defaults
    hpxml = _create_hpxml('base.xml')
    hpxml.air_infiltration_measurements[0].infiltration_volume = 25_000
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_infiltration_values(hpxml_default, 25_000)
  ...
  def test_water_heaters
    ...
    hpxml.water_heating_systems.each do |wh|
      ...
      wh.heating_capacity = 15_000.0
      wh.tank_volume = 40.0
      wh.recovery_efficiency = 0.95
    end
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_water_heater_values(hpxml_default, [true, 15_000.0, 40.0, 0.95])
    ...
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_water_heater_values(hpxml_default, [false, 18_766.7, 50.0, 0.98])
    ...
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_water_heater_values(hpxml_default, [false, 18_766.7, 66.0, 0.98])
    ...
    XMLHelper.write_file(hpxml.to_oga, @tmp_hpxml_path)
    hpxml_default = _test_measure
    _test_default_water_heater_values(hpxml_default, [false, 15_354.6, 50.0, 0.98],
                                      [false, 36_000.0, 40.0, 0.756])
  end
  ...
  def _test_default_lighting_values(hpxml, interior_usage_multiplier, garage_usage_multiplier, exterior_usage_multiplier, schedules = {})
    ...
    if !schedules[:grg_month_mult].nil?
    ...
    if !schedules[:hol_begin_day_of_month].nil?
    ...
    if !schedules[:hol_end_month].nil?
    ...
    if !schedules[:hol_end_day_of_month].nil?
  ...
  def _create_hpxml(hpxml_name)
    HPXML.new(hpxml_path: File.join(@sample_files_path, hpxml_name))
  end
end
