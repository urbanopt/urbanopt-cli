...
class HPXMLTest < MiniTest::Test
  ...
  def _get_results(rundir, sim_time, workflow_time, annual_csv_path, xml)
    ...
    unless xml.include? 'ASHRAE_Standard_140'
  ...
  def _verify_outputs(_runner, rundir, hpxml_path, results)
    sql_path = File.join(rundir, 'eplusout.sql')
    assert(File.exist?(sql_path))
    ...
    File.readlines(File.join(rundir, 'eplusout.err')).each do |err_line|
      ...
      # HPWHs
      unless hpxml.water_heating_systems.select { |wh| wh.water_heater_type == HPXML::WaterHeaterTypeHeatPump }.empty?
      ...
      # HP defrost curves
      unless hpxml.heat_pumps.select { |hp| [HPXML::HVACTypeHeatPumpAirToAir, HPXML::HVACTypeHeatPumpMiniSplit].include? hp.heat_pump_type }.empty?
      ...
      unless hpxml.cooling_systems.select { |c| c.cooling_system_type == HPXML::HVACTypeEvaporativeCooler }.empty?
      ...
      unless hpxml.cooling_systems.select { |c| c.cooling_system_type == HPXML::HVACTypeRoomAirConditioner }.empty?
    ...
    hpxml.frame_floors.each do |frame_floor|
      ...
      # Tilt
      hpxml_value = if frame_floor.is_ceiling
                      0
                    else
                      180
                    end
    ...
    hpxml.heating_systems.each do |heating_system|
      ...
      if htg_sys_type == HPXML::HVACTypeBoiler
        next unless hpxml.water_heating_systems.select { |wh| [HPXML::WaterHeaterTypeCombiStorage, HPXML::WaterHeaterTypeCombiTankless].include? wh.water_heater_type }.empty? # Skip combi systems
    ...
    # HVAC Load Fractions
    unless hpxml_path.include? 'location-miami'
    ...
    # Water Heater
    unless hpxml.water_heating_systems.select { |wh| [HPXML::WaterHeaterTypeCombiStorage, HPXML::WaterHeaterTypeCombiTankless].include? wh.water_heater_type }.empty?
    ...
    unless (fan_cfis + fan_sup + fan_exh + fan_bal + vent_fan_kitchen + vent_fan_bath).empty?
      mv_energy = UnitConversions.convert(results['Electricity: Mech Vent (MBtu)'], 'MBtu', 'GJ')

      unless fan_cfis.empty?
    ...
    # Lighting
    ltg_energy = results.select { |k, _v| k.include? 'Electricity: Lighting' }.map { |_k, v| v }.sum(0.0)
  ...
  def _write_summary_results(results_dir, results)
    ...
    CSV.open(csv_out, 'w') do |csv|
      ...
      results.sort.each do |xml, xml_results|
        csv_row = [xml]
        output_keys.each do |key|
          csv_row << if xml_results[key].nil?
                       0
                     else
                       xml_results[key]
                     end
